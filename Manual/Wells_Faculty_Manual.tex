...
%\newcommand{\editReplace}[2]{\textcolor{blue}{#2}\footnote{\textcolor{blue}{Replaced}: #1}}
\newcommand{\editReplace}[2]{\textcolor{blue}{#2}}
...
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%____% a. 
\item The Dean of the College may, \editRemove{at \editReplace{his or her}{their} discretion, and}\editNote{Removed phrase redundant with `may'} in consultation with the Advisory Committee, appoint an Associate Provost of the College, who shall assist in the execution of the duties and responsibilities of the Dean.
%____% b. 
...
%___% 1. 
\paragraph{\editAdd{Regular meetings}} 
...
%__% B. Agenda
\subsubsection{\editAdd{Notice of} Agenda}

%___% 1. 
\paragraph{\editAdd{Regular Business}} 
...
%___% 2. 
\paragraph{\editAdd{Emergency Business}} 
...
Assistant to the President\editReplace{,}{;} 
Associate Director of Admissions\editReplace{,}{;} 
Associate Provost\editReplace{,}{;} 
Chief Operating Officer\editReplace{,}{;} 
Dean of Students\editReplace{,}{;} 
Director of Alumnae Relations\editReplace{.}{;} 
Director of Communications and Marketing\editReplace{,}{;} 
Director of Experiential Learning and Career Services\editReplace{,}{;} 
Director of Financial Aid\editReplace{,}{;} 
Director of \editReplace{Off-Campus Study}{International Programs}\editReplace{,}{;} 
Librarians for Public Services and Outreach, Technical Services and System Management, and Circulation and Reserves\editReplace{,}{;} 
Vice President for Advancement\editReplace{,}{.} 
\end{enumerate}
\end{enumerate}
...
%___% 1. 
\paragraph{\editAdd{Distribution}} 
...
\begin{enumerate}[label=\itemLevelB,ref=\itemRefB]
%_____% 1) 
\item The voting members of each division, as per \cref{sec:FacultyMeetings}.\labelcref{sub:Procedure}.\labelcref{par:Voting} above, shall elect a chairperson for a term of two academic years with the possibility of re-election for a second consecutive two-year term. Willingness to stand for election as division chair shall be taken as indicating willingness to serve the entire term. No faculty member may serve again as division chair until one year \editReplace{has}{shall have} elapsed since the end of \editReplace{his or her}{the} previous term of service.
%_____% 2) 
...
%______% c) 
\item Each division chair shall report to \editReplace{his or her}{their} division on all matters discussed at the Dean's Council that affect the work of the division and shall seek the advice of the division on matters of policy and decision-making affecting the division.
\end{enumerate}
...
The faculty of each major program shall elect a chair for a period of two years \editReplace{with the possibility of re-elections}{who may be re-elected any number of times}\editNote{Had said ``with the possibility of re-elections''}. In the case of major programs with only one full-time faculty member, that faculty member shall act as chair.
%____% b. 
...
\paragraph{\editAdd{Primacy and Service Expectation}}
...
%___% 3. 
\paragraph{\editAdd{Outside Experts}}
...
\paragraph{\editAdd{Eligibility and Service}}
...
%___% 2. 
\item Unless otherwise specified, terms of office on committees shall be three years. Terms shall be \editReplace{so arranged}{staggered such} that there is an annual rotation within the committee.

%___% 3. 
\item No faculty member may serve again on a given committee until one year \editReplace{has}{shall have} elapsed since \editReplace{his or her}{the} previous full term.

%___% 4. 
\item Ordinarily, no faculty member \editReplace{should}{shall} serve on more than one elected committee.
\end{enumerate}

%___% 5. 
\paragraph{\editAdd{Exclusivity}}
...
%___% 2. 
\paragraph{\editAdd{Dean}}
The Dean of the College shall chair the Curriculum Committee\editReplace{ and is}{, shall be} a member of the Educational Policy Committee and shall be a member \exoff of all faculty committees except the Advisory Committee on Faculty Personnel, the Committee on Committees, the Evaluation Committee and the Review Committee.
...
%___% 1. 
\paragraph{\editAdd{Formation}}
All standing committees shall be empowered to form sub-committees to further specific aims. 

\paragraph{\editAdd{Membership}}
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%___% 2. 
\item At least one member of the standing committee \editReplace{must}{shall} be a member of the sub-committee.
...
%___% 2. 
\item If the Faculty creates an \adho committee, \editReplace{it shall inform the President and Dean}{the President and Dean shall be informed} promptly in writing, defining the function of the committee.

%___% 3. 
\item If the President or Dean creates an \adho committee, \editReplace{she or he shall inform the Faculty}{the Faculty shall be informed} promptly in writing, defining the function of the committee.
\end{enumerate}

%___% 4. 
\paragraph{\editAdd{Franchise}}
...
%____% a. 
\item \editReplace{Any}{All} officer\editAdd{s} of instruction holding the rank of Professor, Associate Professor, Assistant Professor, \editRemove{Instructor}\editNote{Removal of `Instructor'} or Lecturer teaching at least half-time (as determined by the Dean of the College) shall be considered eligible for committee service, provided that \editReplace{he or she has}{they have} taught at the College for at least one year and that one year has elapsed since \editReplace{his or her}{the} previous full term of service on that committee.
%____% b. 
...
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%____% a. 
\item Results of election of division chairs shall be conveyed to the Committee on Committees no later than two weeks prior to the close of classes in the Spring \editReplace{Term}{Semester}.
%____% b. 
\item At least two weeks before the \editReplace{end}{close} of classes in \editReplace{May}{the Spring Semester}, the Committee on Committees shall initiate the process described in \cref{sub:Elections}.\labelcref{par:GeneralRegulations}.\labelcref{item:ballots05}. above. Any \editReplace{person}{faculty members} wishing to withhold \editReplace{his or her}{their} name from nomination must inform the Secretary of the Faculty in writing.
%____% c. 
...
%___% 1. 
\paragraph{\editAdd{Protected matters}}
...
%___% 2. 
\paragraph{\editAdd{Unprotected matters}}
All other matters shall be considered to be non-confidential.

%___% 3. 
\paragraph{\editAdd{Reporting}}
...
The Committee shall oversee the application of academic regulations regarding standing, deficient students, re-admission, requirements for the undergraduate degree, academic honors and prizes, student advising, and \editAdd{shall} propose new policy as necessary.
...
%_____% 2) 
\item The Committee shall meet informally with the Faculty at the beginning of each semester to discuss matters of general concern on which the Committee plans to deliberate during that \editReplace{term}{semester}. Recommendations made by the Committee on matters of general concern to the Faculty shall have the approval of the Faculty.
%_____% 3) 
...
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%____% a. 
\item Membership\editReplace{}{, Length of Term}
...
The Curriculum Committee shall consist of the Dean of the College as Chairperson, the chairpersons of the four divisions, the Registrar as secretary, and one student. \editAdd{A member of LIS chosen by the Dean and the Dean of Academic Advising shall serve as non-voting advisors.}\editNote{Adding Dean of Academic Adivsing as an advisor.}\editNote{ComOnComs check}
...
The Inclusive and Intercultural Excellence and Off Campus Study Committee shall consist of three members of the Faculty, the Director of \editReplace{Off-Campus Study}{International Programs}\editRemark{Recheck}{, and the Director of Institutional Diversity}{Atrophied position?}. A faculty member shall serve as chair.
%____% b. 
\item Responsibilities
\begin{enumerate}[label=\itemLevelB,ref=\itemRefB]
%_____% 1) 
...
The duties, membership, and procedures of this Board are described in the \editReplace{Wells Collegiate Constitution}{\constitution}. Its Faculty/Administration members are the Dean of the College, the Dean of Students, and the three faculty members of the Academic Standing and Advising Committee.
...
Further information about the nature and functions of the Community Court can be found in the \editReplace{Wells College Constitution}{\constitution}.
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%____% a. 
...
The Community Court shall meet upon the request of a member of the student body, the Faculty or the Administration, to hear and decide cases of alleged violations of Community Honor as defined by the \editReplace{Wells College Collegiate Bylaws and Constitution}{\constitution}.
\end{enumerate}

%___% 3. The Judicial Appeals Board
\paragraph{The Judicial Appeals Board}

Further information about the nature and functions of the Judicial Appeals Board can be found in the \editReplace{Wells College Constitution}{\constitution}.
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%____% a. 
\item Membership, Length of Term
...
%__% A. 
\subsubsection{\editAdd{Participation}}
...
%__% B. 
\subsubsection{\editAdd{Representation}}
...
%__% B. 
\subsubsection{\editAdd{Professional Responsibility}}
...
%__% C. 
\subsubsection{\editAdd{Academic Freedom}} \label{sub:AcademicFreedom}
Wells College endorses the 1940 \href{http://www.aaup.org/AAUP/pubsres/policydocs/contents/1940statement.htm}{Statement on Academic Freedom and Tenure} of the American Association of University Professors. All members of the Faculty (See \cref{art:FacultyAndGovernance}, \cref{sec:Personnel}\editReplace{}{.\labelcref{sub:Faculty}}) shall have full freedom of inquiry; of instruction subject to the educational plans of the Faculty and of their respective divisions; and of expression of opinion. A faculty member's exercise of \editReplace{his}{the} rights and duties as a citizen and member of a community shall in no way prejudice \editReplace{his}{their} academic status. In enjoying these rights a faculty member must recognize the corresponding obligation to accept full responsibility for \editReplace{his}{their} utterances and behavior.
...
%___% 2. 
\paragraph{\editAdd{Tenure-Track}} 
...
%__% B. 
\subsubsection{\editAdd{Contingent}}
...
%____% d. 
\item If the Dean agrees with the Search Committee's recommendation, the Dean shall forward \editReplace{his or her}{their} recommendation and the recommendation of the Search Committee to the President.
...
%___% 6. 
The decision to designate a position as tenure-track shall have been made before hiring, not at the time of an interim reappointment or at the time of the tenure decision. However, the identification of a position as tenure-track does not imply that \editAdd{the individual} appointed to that position shall qualify for tenure, but rather that \editReplace{he or she}{the individual} is eligible for consideration for tenure.

%_% Section 4. Academic Ranks
\subsection{Academic Ranks}\label{sec:AcademicRanks}

%__% A. 
\subsubsection{\editAdd{Appointments}}
...
\paragraph{\editAdd{Eligibility and Conditions}}
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]

%___% 1. 
\item Any tenured officer of instruction may apply for sabbatical leave, provided \editReplace{he}{they} will have completed six years of full-time service to the College prior to the year for which leave is being requested (except as provided for below.)
...
%__% A. 
\subsubsection{\editAdd{Notification}}
An officer of instruction is expected to notify the Dean of the College of \editReplace{her or his intended retirement or resignation}{the intent to retire or resign} by the end of the first (fall) semester of the prior academic year, if possible.

%__% B. 
\subsubsection{\editAdd{Emeriti}}
A faculty member who has retired from Wells College in good standing may be awarded the title of Professor Emerita/Emeritus, provided that \editReplace{she or he has}{they have} been employed full time by the College for at least ten years either as an associate professor or as a professor. The initiative for proposing emeritus status may originate with the President or the Dean of the College or the Advisory Committee. The designation requires the approval of the Dean of the College and the President and is awarded upon the vote of the Board of Trustees.
...
\subsubsection{\editAdd{Full Time Officers of Instruction}}
%__% A. 
...
%__% C. 
\subsubsection{\editAdd{Part Time Officers of Instruction}}
...
%3.
\item \editReplace{A student must take at least 6 courses (3-4 semester hours each) in \their \modified{5/13/93} major at Wells College or in an affiliated program, and at least 4 courses (3-4 semester hours each)   for a minor at Wells College.}{At least 6 courses (3-4 semester hours each) in a major must be taken at Wells College or through an affiliated program. At least 4 courses (3-4 semester hours each) for a minor must be taken at Wells College.}\editRemark{Clarification}{}{had been ambiguous whether the courses had to be taken at Wells, or simply for a major at Wells}
%4.
\item Each student, whether first-year or transfer student, is expected to satisfy College requirements, the requirements of \editReplace{\their}{the} major, and the requirements \modified{4/8/97} of \editReplace{\their minor}{any minors} as stated in the \editReplace{catalogue}{catalog} in effect at the time \editReplace{\they first matriculates}{of first matriculation} at Wells College. \editReplace{If, however, a student}{A student who} ceases to attend the College for two years or more\editRemove{, \they} will be expected to meet \editReplace{major and minor}{the} requirements as stated in the \editReplace{catalogue}{catalog} in effect at the time \editReplace{\they returns}{of return}.
%5.
...
%6.
\item Each student will maintain a \editAdd{minimum}\editRemark{Clarification}{}{had implied exactly a 2.0 would be maintained} Grade Point Average of 2.0 for all \editRemove{\their} Wells and Wells-affiliated courses. \editAdd{See \cref{art:RegulationsConcerningStudentStatus}.\labelcref{sec:AcademicRegulations}.\labelcref{sub:AcademicStanding}.\labelcref{par:UnsatisfactoryProgress} for the consequences of grade point averages which fall below 2.0.}
%7.
\item Students may earn no more than 4 semester hours of credit during \editAdd{any given}\editRemark{Clarification}{}{had been unclear whether it was 4 across all sessions total, or 4 per session} January  Intersession.\modified{2/8/05}
\end{enumerate}
...
Each \modified{5/13/93}  student must meet the course requirements for a major, including a comprehensive evaluation in \editReplace{\their}{the} major, and maintain at least a 2.0 GPA for all Wells or Wells-affiliated courses taken for \editReplace{\their}{the} major field during \editReplace{\their}{the} sophomore, junior and senior years. No major will require more than 65 semester hours.
...
\paragraph{Degree Expected}
 ...
 Except\modified{9/9/97}  for students in 3/2 or 3/4 articulation programs, a student with senior standing may petition the Committee on Academic Standing and Advising to participate in commencement activities if \editRemove{\they will have completed} all requirements for the major, the senior comprehensive evaluation, and at least 114 semester hours \editAdd{will have been completed}. \editReplace{\They}{The student} would be expected to complete \editReplace{\their}{the} remaining work by the end of the calendar year. A student in a 3/2 or 3/4 articulation program will be expected to follow the terms of the program and may participate in commencement activities when \editReplace{\their}{the} work at Wells College is completed.

%Section 2. Master of Arts
\subsubsection{Master of Arts}

\paragraph{\editAdd{Admission}} Admission to graduate study does not constitute admission to candidacy for a Master of Arts degree. Application for candidacy for the Master of Arts degree must be made upon the accumulation of twelve hours of graduate credit, transferred to or earned at Wells College.
...
%A.
\paragraph{\editAdd{Responsibility}}
...
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
%1.
\item At any time after elevation to sophomore standing, but no later than\modified{11/12/01} March 1 in \editReplace{\their}{the} sophomore year, a student shall declare a major.

\item To be eligible to declare the major, the student must have a GPA of 2.0\modified{5/13/93} in course work in the proposed major. If \editRemove{\they has} fewer than 6 semester hours in \editReplace{\their}{the} proposed major \editAdd{have been completed}, a student may petition for provisional approval contingent on a GPA of 2.0 in course work in the proposed major at the end of \editReplace{\their}{the} sophomore year. A transfer student with junior standing shall declare a major by the end of \editReplace{\their}{the} first semester; formal admission into the major requires the written approval of a Wells College faculty member in the major based on evaluation of \editReplace{\their}{the} transfer record.

\item Students are not normally encouraged to elect double majors. If a student \modified{10/17/00} feels \editReplace{\their}{that their} plans necessitate a double major rather than a major and a minor, \editRemove{\they must file} a Double Major Proposal form \editAdd{must be filed} with the Registrar no later than the \modified{9/9/2003}
%last day of classes of the junior year (Proposed May 2003)  -BADAMS via DKOESTER
end of advising week of the first semester of \editReplace{\their}{the} junior year. This Proposal must indicate how the student's proposed plan of study meets the College's goals for a sound liberal arts program, including courses that satisfy the Core Requirements. The Proposal must be approved by an advisor from each major. The student must have a cumulative grade point average of at least 3.5 at the time of the proposal. A double major may not include an individualized major. Final approval rests with the Academic Standing and Advising Committee.

%2.
\item A student desiring a minor shall declare the minor on a Declaration of Minor\modified{4/8/03} form, available in the Registrar's office or online, no later than\modified{5/08/12} the last day of classes of the first semester of \editReplace{\editReplace{his or her}{\their}}{the} senior year. The declaration must be approved by the student's major advisor and the faculty coordinator for the minor.

\item A student may request to complete two minors by filing two minor declaration forms, available in the Registrar's Office or online, no later than the last day of classes of the first semester of \editReplace{\editReplace{his or her}{\their}}{the} senior year. Each form must be approved by the student's major advisor and the faculty coordinator for the minor.
\end{enumerate}
...
\begin{enumerate}[label=\itemLevelB,ref=\itemRefB]
%1. 
\item \label{iitem:passing} Passing \modified{9/14/99} a course in the Liberal Arts with a grade of C- or better, taken from an accredited college or university. Students entering Wells College under an articulation agreement \editReplace{with}{will} be granted credit according to the agreement. (See \cref{art:RegulationsConcerningStudentStatus}, \cref{sec:AdmissionOfStudentsToAdvancedStanding}.\labelcref{sub:AssignmentOfCredit})
%2.
...
%4.
\item \label{iitem:clep} Passing a CLEP student examination in a subject covered in the Wells College curriculum with at least the minimum score recommended by the American Council of Education. Normally no more than two courses (6\editAdd{-}8 semester hours) of such credit may be applied toward a Wells degree.
%5.
...
\item In all cases, placement in advanced coursed and applications to the requirements for the major will be at the discretion of the major field \editReplace{coordinator}{advisor}\editRemark{Correction}{}{had said coordinator}.
...
A student may accelerate \editReplace{\their}{the} normal course program with the object of graduating in less than four (4) years by submitting a program for approval to the Dean of the College.
...
%G.
\paragraph{\editAdd{Language}}
 A student is expected to use clear and idiomatic English in all classroom work and examinations. If found deficient in this respect, \editRemove{\they may be required to take} extra work in English composition under the direction of the Department of English \editAdd{may be required}. This extra work shall not count towards the degree.

%H.
\paragraph{Auditing Courses}
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
\item A student may visit a course on mutual agreement \editReplace{between \them and}{with} the instructor. 
\item \editReplace{If \they}{A student who} registers for an ``audit''\editRemove{, \they} must participate actively in the course and must complete all work specified \editRemove{for \them} by the instructor at the beginning of the course.
\item  A student desiring to audit a course shall get permission of both \editReplace{\their}{the} faculty advisor and the instructor of the course.
\item Under either of the following circumstances, a student must petition the Dean of the College: \editRemove{if \they wishes} to audit more than one course in a semester or if \editReplace{\their}{the} request to audit a course is made after the first week of classes. 
...
%I.
\paragraph{\editAdd{Off-Campus Study}}
 Any student whose academic progress will be advanced by a semester or a year spent at another college or university, and whose record testifies to \editReplace{\their}{the} ability to profit from such study, may request permission to undertake it by applying to the \editReplace{Committee on Off-Campus Study}{Inclusive and Intercultural Excellence and Off-Campus Study Committee}\editRemark{Update}{}{committee name has changed}.

%J.
\paragraph{\editAdd{Summer credits}}
...
%A.
\paragraph{\editAdd{Selection}}
Schedules, showing courses selected for the oncoming semester, shall be submitted at times designated by the Registrar. \editReplace{Each schedule submitted shall bear the signature of the academic advisor}{The student may submit the schedule only after being granted permission by their Academic Advisor}\editRemark{Update}{}{to current practice - had previously required signatures on schedules}.

%B.
\paragraph{\editAdd{Add/Drop}}
...
%C.
\paragraph{\editAdd{Late Registration}}
Each student is responsible for submitting \editReplace{\their}{a} semester program and schedule on or before the deadline fixed by the Registrar. A late registration service fee will be incurred by any student who does not meet the stated deadline unless \editRemove{\they is} exempted by the Dean of the College upon written request of the student, stating the reasons. A receipt for the fee \editAdd{or an email} from the Treasurer's Office must \editReplace{accompany late registration programs and schedules}{be submitted before the hold is released}\editRemark{Clarification}{}{by Registrar}. The fee does not apply to revisions of previously submitted semester programs and schedules that are made during the time periods established for such revisions.
...
Any paid internship must be approved by the faculty sponsor, faculty coordinator of internships, and the \editAdd{Director of Experiential Learning and Career Services}\editRemark{Update}{}{job title}.
...
\item The purpose of Independent Study is to supplement the more structured methods of regular courses with the opportunity for the student of high initiative and responsibility to apply \editReplace{\their}{their} abilities to new material with a minimum of guidance. 

\item While there are no formal course prerequisites for such work, it is the normal expectation that independent projects will involve explorations in depth of some prior work, or work for which \editReplace{\they}{the student} has special non-academic background that qualifies \editReplace{\them}{them}\modified{10/10/89} for the study proposed. Exceptions may be made in cases where a student of generally demonstrated capability wishes to do exploratory work in a field in which \editReplace{\they has}{they have} little or no background, in a manner not provided for by the regular curriculum. 
...
\item A student who has a plan for Independent Study should consult with the instructor under whose guidance \editReplace{\they proposes to work}{the work is proposed}. If the instructor approves the project, a brief description of the project and the amount of credit proposed for it, approved by the instructor and advisor, must be submitted to the Registrar with the program of study during the registration period preceding the semester in which the work is to be conducted. 
...
\paragraph{\editAdd{Grading and Credit}}
\begin{enumerate}[label=\itemLevelA,ref=\itemRefA]
...
\item Distinction: The degree will be awarded \textit{with distinction in the major field }to each student who is recommended for the award by the faculty in \editReplace{\their}{the} major field because \editReplace{\they}{the student} has shown:
...
%(3)
\item and distinction in \editReplace{\their}{the} comprehensive evaluation.
\end{enumerate}
...
\item At the discretion of the instructor, grades may be designated as Pass with Distinction (PD), Satisfactory (S), Fail (F), but such designation must be included in the \editReplace{catalogue}{catalog} description of the course. In the case of internships, all internships must be graded Credit/Fail.
...
In case of a conspicuous failure in a final examination, when the student, in the opinion of the instructor, fails to show anything like a satisfactory comprehension of the subject, the student may be marked as failing even though the numerical value of \editReplace{\their}{the} class work would \editReplace{give \them}{yield} a grade of D-.
...
%b.
\item  In each \editReplace{long term}{semester}, final grades shall be submitted \editRemove{in writing} to the Registrar within five days following the course examination. Grades for courses without a final examination are due within five days after the end of the examination period. Final grades for Seniors in the Spring Semester shall be reported to the Registrar at the time \editReplace{\editReplace{s/he}{\they} specifies}{specified}.
...
%d.
\item  If an instructor \modified{4/9/96}wishes to change a recorded grade, \editRemove{\editReplace{he}{\they} will submit to the Dean of the College} a written request for the change \editAdd{will be submitted to the Dean of the College}. Except under extraordinary circumstances, as determined by the Dean, grades will be changed only through the eighth week of the subsequent semester and only because of the instructor's corrected calculation of the grade or because of the instructor's decision to consider ``lost'' work submitted by the student. Upon approval by the Dean, the change will be recorded by the Registrar. The Registrar will notify the student, in writing, within two weeks of the change in the recorded grade.
\end{enumerate}
\end{enumerate}
...
\item Should a student find it necessary to take an examination \textit{in absentia, }\editReplace{she}{the student} will be responsible for making necessary arrangements in consultation with \editReplace{\their}{the} instructor. If the instructor requests, the student will specify the time and place the examination will be given and/or the name, address and telephone number of an individual who will administer the examination.
\end{enumerate}
\end{enumerate}
...
In consultation with its \editRemove{new}\editRemark{Update}{}{Had said 'new majors' - but Juniors are no longer the 'new majors'} majors and with the approval of the \editReplace{Academic Program and Policy Committee}{Curriculum Committee}\editRemark{Update}{}{was APPC}, each department or programmatic area shall  set forth the means by which students then in the junior class will meet the goal of comprehensive evaluation.

%3.
\item Report of Grades

Each department or programmatic area shall submit to the Registrar a grade of Distinction, Satisfactory, or Fail for each student. The faculty will report to each student the result of \editReplace{\their }{the} comprehensive evaluation.
...
Each instructor shall keep a record of graded work of the students in each of \editReplace{\editReplace{her/his}{\their}}{their} courses for at least three years.
...
\item The Registrar shall keep a file of course syllabi \editRemove{and midyear (for hyphenated courses)} and final examinations for all courses for at least three years. Each instructor is responsible for submitting this material to the Registrar each \editReplace{term}{semester}.
...
\item Under the provisions of the federal Family Educational Rights and Privacy Act of 1974, a student may review College records, files, and data directly related to \editReplace{\themself}{themselves} except: 
...
%3. 
\item records to which \editReplace{\they has waived}{the} right of access \editAdd{has been waived}; 

%4. 
\item and parents' financial records.
\end{enumerate}

\item Any student may seek correction or deletion of records \editReplace{\they}{which the student} feels are inaccurate, misleading, or in violation of \editReplace{\their}{their} rights. \editReplace{\They}{The student} will initiate such action by consulting the Dean of the College for records concerning academic matters or the Dean of Students for records concerning non-academic matters.

\item Information from the records of a student who is a dependent of \editReplace{\their}{their} parents for Internal Revenue Service purposes may be released to the parents without the student's consent. Unless a student has filed notification of financial independence and supporting evidence of \editReplace{\their}{that} independence with the Dean of Students by the first day of classes of each academic year, \editReplace{\they}{they} will be assumed to be dependent.
...
\item A student's permanent record will include a record of all courses and grades, honors received, and participation in officially recognized activities. All other materials relating to \editReplace{\their}{the students's} academic performance will be destroyed within one year following graduation.
...
\paragraph{\editAdd{Liberal Arts Credit}}
...
%D.
\paragraph{\editAdd{Non-Liberal Arts Credit}}
...
%E.
\paragraph{\editAdd{Placement in Advanced Courses}}
Placement in advanced courses will be at the discretion of the department involved with the understanding that a student can not take for credit a Wells course that substantially duplicates a transfer course for which \editReplace{\they}{the student} has credit. Applicability of any transfer courses to requirements for the major will be at the discretion of the department.
...
%B.
\paragraph{\editAdd{Deadlines}}
Where permission to complete missed or late work has been granted by the faculty member, \editReplace{\editReplace{he or she}{\they}}{the faculty member} will set a deadline for the completion of the work and will determine whether or not a penalty is to be exacted for lateness. Once the deadline has been set no further extensions save for exceptional circumstances will be allowed and the student alone will be responsible for seeing to it that \editReplace{\their}{all} obligations are fulfilled as stipulated. In no case may deadlines be set beyond the times specified in the Manual for turning in grades to the Registrar (see \cref{art:RegulationsConcerningStudentStatus}, \cref{sec:AcademicRegulations}.\labelcref{sub:CourseGrades}.\labelcref{par:DeterminationOfGrades}.\labelcref{item:ReportOfGrades}).

%C.
\paragraph{\editAdd{Honor Suspension}}
...
\begin{enumerate}[label=\alph*)]
\item A leave of absence is granted to a student who must be absent during the semester but anticipates returning to complete \editReplace{\their}{the} course work. A leave of absence for medical or other personal reasons may be granted by the Dean of Students for a maximum of 15 days; only one such leave may be granted during a 12-month period. When a student initiates the leave of absence, the Dean of Students Office will notify \editReplace{\their}{the} academic advisor and the faculty in whose courses \editReplace{\they}{the student} is registered.
...
\item When a student is on extended leave of absence, Wells College cannot certify enrollment for \editReplace{\them for} any semester during which \editAdd{the student} is on such leave.
\end{enumerate}
...
%a.
\paragraph{\editAdd{Affiliated Programs}}
...
%b.
\paragraph{\editAdd{Field Experience}}
...
%c.
\paragraph{\editAdd{Non-Affiliated Programs}}
...
A student withdraws when \editReplace{\they is not planning to}{intending to not} return to Wells College.

%A.
\paragraph{Procedure}
\begin{enumerate}[label=\alph*)]
\item A student wishing to withdraw from the College before the completion of a semester is required to comply with the withdrawal procedure. This process is initiated with the Dean of Students and requires completion of the withdrawal form. Students may not withdraw after the last day of classes. When a student has officially withdrawn, the Dean of Students Office will notify \editReplace{\their}{the} academic advisor and the faculty in whose courses \editReplace{\they}{the student} was registered.

\item A student who is completing the current semester but is not intending to return to Wells for the next semester should also complete a withdrawal form and indicate \editReplace{\their}{the} reasons for leaving the College.
\end{enumerate}
%B.
...
%4.
\item Senior Citizen: Anyone over the age of \editReplace{62}{55} may \editReplace{take up to two classes a semester}{audit one class a semester free of charge}\editRemark{Change}{}{based on information from the Registrar - was "take up to two classes a semester", with no mention of 'free'}.

\oldbreak{X-1}

%5.
\item \label{item:employeeClasses} Wells College Employee: A Wells College employee is eligible to take up to 32 semester hours with permission from the appropriate senior staff. \editReplace{Female employees may}{Employees may also} seek admission into a degree program.

%6.
\item \label{item:spouseClasses} Child/Spouse of a Wells College Employee: The child or spouse of an employee is eligible for full-time enrollment up to 32 semester hours, after which time \editReplace{daughters and wives}{they}\editRemark{Update/change}{}{current Manual restricts to females only} may apply for admission to the College in a degree program.
